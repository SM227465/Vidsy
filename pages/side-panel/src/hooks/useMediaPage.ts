...
export const useMediaPage = () => {
  ...
  const [forceDetecting, setForceDetecting] = useState(false);
  const [forceDetectDone, setForceDetectDone] = useState(false);

  useEffect(() => {
    chrome.tabs.query({ active: true, currentWindow: true }).then(([tab]) => setTabId(tab?.id ?? null));
  ...
  const onDownload = async (item: MediaItem, outputFormat?: 'mp4' | 'mp3') => {
    ...
    const chosenVariant = item.variants?.length
    ...
    if (!res?.ok) setDownloadState({ busyUrl: null, error: res?.error ?? 'Download failed' });
    else setDownloadState({ busyUrl: null, error: null });
  };
  ...
  const forceDetect = async () => {
    if (!tabId || forceDetecting) return;
    setForceDetecting(true);
    try {
      await chrome.runtime.sendMessage({ type: 'media/force-detect', tabId });
    } catch {
      /* ignore */
    } finally {
      setForceDetecting(false);
      setForceDetectDone(true);
      setTimeout(() => setForceDetectDone(false), 1800);
    }
  };
